...
describe("Services Page", () => {
  ...
  test("should render the services page", () => {
    //Titles
    expect(
      screen.getByText("Sesiones de Psicooncología Individual")
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByText("Asesoramiento Institucional en manejo de Duelo")
    ).toBeInTheDocument();

    //Content
    expect(
      screen.getByText(
        "Un espacio personalizado, cálido y confidencial para explorar tus emociones y necesidades durante el proceso oncológico, brindándote herramientas para tu bienestar. Terapia virtual desde la comodidad de tu hogar."
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
