...
export default function RegisterFormScreen({
  ...
  const formSections = [
    {
      fields: ["name", "age", "identity_document"],
  ...
  const onSubmit = async (values: SubscriberWithHowDidYouHear) => {
    ...
    if (
      data.error &&
      data.error.toString().includes("Ya estás inscripto en este recurso")
    ) {
  ...
  const nextStep = async () => {
    const fieldsToValidate = currentSection.fields as Array<
      keyof z.infer<typeof SubscriberSchema>
    >;
  ...
  if (loading) {
    return (
      <SplashLoaderModal
        open={loading}
        message="Un momento... estamos procesando tus datos ✅"
      />
    );
  }
  ...
  return (
    <Suspense
      fallback={<SplashLoaderModal open={loading} message="Cargando..." />}
    >
      <div className="flex min-h-screen flex-col items-center bg-gray-50 p-4 md:p-8">
        <div className="w-full max-w-3xl h-full">
          <div className="flex flex-col text-center justify-center h-2/4">
            ...
            <div>
              <h1 className="text-2xl xs:text-4xl font-semibold text-gray-800 mb-4">
                {resource?.name}
              </h1>
            </div>
          </div>
