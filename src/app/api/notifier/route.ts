...
export async function POST(req: Request) {
  const body: { subscriber: Subscriber; resource: Resource; type: string } =
    await req.json();

  const emailContentByType = new Map([
    [
      "welcome",
      getWelcomeEmail({ subscriber: body.subscriber, resource: body.resource }),
    ],
    [
      "reminder",
      getReminderEmail({
  ...
  try {
    ...
    return NextResponse.json(
      { error: `Error al enviar el email. Error: ${(error as Error).message}` },
      { status: 500 },
    );
  }
}
